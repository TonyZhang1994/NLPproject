...
def get_features(data):
    ...
    for key in feas.keys():
        if key != "verbs":
            colnames.append(key)
    colnames.extend(["verbs", "time_needed", "oven_needed", "microwave_needed"])
...
if __name__ == "__main__":
    ...
    if args.classifier =='LR':
        ...
        # for data in x_train:
        #     training_data.append(get_features(data))
        #training_data = np.array(training_data)
        # print colnames
        print training_data.shape
        # '''data visualization'''
        # trainset = pd.DataFrame(training_data, columns = colnames)
        # trainset["label"] = original_Label
        # plt.figure(1)
        # colors = ['red', 'green', "blue"]
        # pd.plotting.scatter_matrix(trainset.drop(["label"], axis = 1), c = trainset["label"].apply(lambda x: colors[x]), figsize=(15,15))
        # plt.tight_layout()
        # plt.savefig("scatter matrix.png",bbox_inches = 'tight')
        cverror = []
        C = [0.001, 0.01, 0.1, 1, 10, 100]
        for c in C:
            lr = LogisticRegression(penalty = "l2", C = c,  multi_class = "ovr")
            cverror.append(cv_error(lr, dataset=training_data, datalabel=y_train, cv=5))
        ...
        # for data in x_val:
        #     testdata.append(get_features(x_val))
        ...
        for c in C2:
            data2 = word2vec(Data,Size=c)
            data_vec = extrac_vec(data2, Dim=c)
